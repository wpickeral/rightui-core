...
const findAndUnselectPreviouslySelectedTreeItem = (currentNode: HTMLLIElement) => {
    const treeItems = currentNode?.closest(classes.tree)?.querySelectorAll(classes.treeItem) as NodeListOf<HTMLLIElement>;
...
export function TreeItem({
    ...
    const getNextTreeItemThatStartsWithChar = (
        ...
        if (treeItems) {
            const matchingChildren = Array.from(treeItems).filter(
                treeItem => treeItem.textContent?.toLowerCase().startsWith(char.toLowerCase()));
    ...
    const searchWrap = (query: string, currentNode: HTMLLIElement) => {
        ...
        const allTreeItems = currentNode?.closest(classes.tree)?.querySelectorAll(classes.treeItem) as NodeListOf<HTMLLIElement>;

        const matchingNodes = Array.from(allTreeItems).filter(
        ...
        if (nextMatchingNode) {
            ...
            moveFocusToTreeItem(allTreeItems[0] as HTMLLIElement);
        }
    };
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLLIElement>) => {
        e.stopPropagation();
        const treeItems = treeItemRef?.current?.closest(classes.tree)?.querySelectorAll(classes.treeItem) as ArrayLike<HTMLLIElement>;
    ...
    useEffect(() => {
        ...
        if (treeItemRef.current) {
            const getAriaSetSize = () => {
                if (treeItemRef.current) {
                    return treeItemRef.current.closest('ul')?.childElementCount.toString();
                }
            };

            const getAriaPosinset = (node: HTMLLIElement) => {
                if (treeItemRef.current) {
                    const arr = treeItemRef?.current?.closest('ul')?.querySelectorAll(classes.treeItem) as ArrayLike<HTMLLIElement>;
