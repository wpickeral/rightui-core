  ...
  return (
    <Tree label='Items'>
      <TreeItem label="Item 1">
        ...
        <TreeItem content="Item 1.2" />
      </TreeItem>
      <TreeItem content="Item 2">
        <TreeItem content="Item 2.1" />
        <TreeItem content="Item 2.2" />
      </TreeItem>
    </Tree>
  )
}
...
Helper classes are available for styling the components.

| class                     | description                                               |
|---------------------------|-----------------------------------------------------------|
| `uis-tree`                | A `ul` with `role="tree"`.                                |
| `uis-tree__label`         | The label for the tree.                                   |
| `uis_tree-item`           | A `li` element with `role="tree-item"`                    |
| `uis-tree-item__content`  | The child element of the treeitem.                        |
| `uis-tree--subtree`       | A `ul` with `role="group"`.  Represents any nested trees. |
| `uis-tree-item--focus`    | Available when the treeitem has focus                     |
| `uis-tree-item--selected` | Available when the treeitem is selected                   |
| `uis-tree-item--expanded` | Available when the treeitem is expanded                   |
